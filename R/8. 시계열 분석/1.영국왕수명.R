...
#안정적인 시계열이 아닌 데이터: 로그를 이용하거나 차분을 
# 통해 안정적인 시계열로 변환한 후 분석 진행
########################################
#영국왕들의 수명 데이터
...
#arima 모형 생성 - ACF,PACF 차트 또는 auto.arima 함수를 
# 사용하여 최적화된 파라미터 도출
########################################
#이동평균
# 1050+1000의 평균, 1050+1010의 평균, 1010+1020의 평균, 1020+1040의 평균
# 어느 시점의 충격이 이틀간 영향을 미친다면 2차이동평균 
# 모형MA(2) 모델을 추정하게 됨
...
# lag 4부터 점선 안에 존재함. lag 절단값은 4 => AR(3) 앞의 
# 3개 데이터가 현재 데이터에 영향을 줌
########################################
library(tseries)
